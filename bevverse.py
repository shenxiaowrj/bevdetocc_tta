...
class BEVerse(MVXTwoStageDetector):
    ...
    def simple_test(self, img_metas, img=None, future_egomotions=None,
                    rescale=False, motion_targets=None, img_is_valid=None): ## 简单测试的主函数
    ...
    def simple_test_pts(self, x, img_metas, rescale=False, motion_targets=None): ## 简单测试的前向运算函数
    ...
    def aug_test(self, img_metas, img=None, future_egomotions=None,
                 rescale=False, motion_targets=None, img_is_valid=None): ## tta测试的主函数
        '''
        ...
        ## 这是一个检查 检查对图片的两种增强操作的个数 和 图片个数是否相同 如果filp,scale各有两种方式 那么img的个数应该是4 4种batch 4*6=24 共24个图片
        ...
        img_aug_outs = [] ## 建立一个列表存放增强之后的图像结果
        for i in range(len(img)): # 对于每一个batch的img

            ## 获取是否要对bev进行flip
            tta_flip_bev = self.data_aug_conf.get('tta_flip_bev', False)
            if tta_flip_bev:
                # 前后、左右两种翻转方式构成四种组合 [[False, False], [False, True], [True, False], [True, True]] True False 指示是否要进行翻转
                choices = [False, True]
                flip_bev_list = [[d, f] for d in choices for f in choices]
            else:
                flip_bev_list = [[False, False], ]

            bev_outputs = [] ## 建立一个列表存放bev输出
            for flip_bev in flip_bev_list:
                ## 经过网路的以下层 输出bev_feat bev特征
                ...
                ## 将bev特征输入到检测头之中 得到检测结果 我们没有输出检测头 所以最好做特征融合
                # decoder head
                bev_output = self.pts_bbox_head(
                    bev_feat, targets=motion_targets)
                ## 将bev_output flip back回去 准备做融合
                # flip-back outputs
                bev_output = self.flip_bev_output(
                    bev_output, flip_x=flip_bev[0], flip_y=flip_bev[1])
                ## 将得到的bev_output添加到列表里
                bev_outputs.append(bev_output)  ## 如果要做flip_bev,那么就会有四个输出

            ## 对于得到的一种batch的bev输出做聚合
            averaged_bev_output = self.combine_bev_output(bev_outputs)
            img_aug_outs.append(averaged_bev_output) ## 将聚合后的bev特征添加到img_aug_outs列表中,如果要做img的图像增强的话 就会有四个batch输出

        ## 对于得到的batch的输出 进行聚合的 得到最终的bev_output
        bev_output = self.combine_bev_output(img_aug_outs)
        predictions = self.pts_bbox_head.inference( ## 将得到的聚合后的bev_output送入到inference中 得到最后的预测结果
    ...
    def flip_bev_output(self, output, flip_x=False, flip_y=False):
        ...
        for pred_key, pred_val in output[motion_key].items():
            # offset 和 flow 都需要 flipimport torch
...
class BEVerse(MVXTwoStageDetector):
    ...
    def combine_bev_output(self, bev_outputs): ## 将得到的输出 加和 然后做平均 和 PETR 的思路一样
